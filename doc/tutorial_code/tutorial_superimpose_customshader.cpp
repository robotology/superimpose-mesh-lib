...
int main()
{
    ...
    /**
     * We create a SICAD object by passing all the camera parameters, the shader
     * folder path containing the shader code (don't worry, we provide basic
     * shaders as well!).
     * Note that for this simple tutorial we assume that the shader files are
     * (magically) in the same folder of the executable, that's why we use ".".
     *
     * Shader code is included at the end of this snippet code. Have a look at
     * it, it's really simple! If you are not used to it, there are plenty of
     * tutorials online for writing OpenGL shaders.
     **/
